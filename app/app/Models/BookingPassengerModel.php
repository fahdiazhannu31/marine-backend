...
class BookingPassengerModel extends Model
{
    ...
    protected $allowedFields    = ['payment_id', 'name', 'seat_id'];
    protected $useTimestamps    = false; // we set created_at manually

    /**
     * Insert one row per passenger name for a freshly created booking.
     *
     * @param int      $paymentId
     * @param string[] $names
     */
    public function createForBooking(int $paymentId, array $names): void
    {
    ...
    /**
     * All passenger names for a booking, in the order they were entered.
     */
