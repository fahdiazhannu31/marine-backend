...
class ManifestUploadController extends ApiController
{
    ...
    public function baggageTagPdf(int $id)
    {
        ...
        // ── PDF: 220 x 40 mm landscape (to match printer's landscape feed) ───────
        // Printer feeds labels in landscape orientation, so swap width/height
        // Physical label: 40mm wide x 220mm tall → PDF: 220mm wide x 40mm tall
        $pdf = new \App\Libraries\BoardingPassPDF('L', 'mm', [40, 220]);
        $pdf->SetAutoPageBreak(false);
        $pdf->SetTitle('Baggage-Tag-' . $id);
        
        // Small top margin for printer alignment
        $pdf->SetMargins(0, 2, 0);  // 2mm top offset
        ...
        for ($page = 1; $page <= $bagCount; $page++) {
            ...
            $W = 220;   // page width mm (landscape: long side is width)
            $H = 40;    // page height mm (landscape: short side is height)
            ...
            $cx = 5;    // start X position
            $cy = 8;    // center Y position
            $pdf->SetTextColor(0, 0, 0);

            // Column 1: NAMA Marine + Guest Name
            $col1W = 60;
            $pdf->SetFont('Arial', 'B', 8);
            $pdf->SetXY($cx, $cy);
            $pdf->Cell($col1W, 4, 'NAMA Marine', 0, 0, 'L');
            
            $pdf->SetFont('Arial', 'B', 9);
            $guestName = strtoupper($bag['group_name'] ?? '');
            $guestName = strlen($guestName) > 20 ? substr($guestName, 0, 20) : $guestName;
            $pdf->SetXY($cx, $cy + 6);
            $pdf->Cell($col1W, 5, $guestName, 0, 0, 'L');
            
            // Boat name
            $pdf->SetFont('Arial', '', 7);
            $pdf->SetXY($cx, $cy + 12);
            $pdf->Cell($col1W, 4, strtoupper($boatName), 0, 0, 'L');
            ...
            $pdf->SetFont('Arial', '', 6);
            $pdf->SetXY($cx, $cy);
            $pdf->Cell($col2W, 3, $tripDateFmt, 0, 0, 'L');
            
            $pdf->SetXY($cx, $cy + 4);
            $pdf->Cell($col2W, 3, $dirLabel, 0, 0, 'L');
            
            $routeText = strtoupper($origin) . '-' . strtoupper($destination);
            $pdf->SetXY($cx, $cy + 8);
            $pdf->Cell($col2W, 3, $routeText, 0, 0, 'L');
            
            // Destination
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->SetXY($cx, $cy + 13);
            $pdf->Cell($col2W, 4, strtoupper($destination), 0, 0, 'L');
            
            $cx += $col2W + 5;

            // Column 3: Bag info (right aligned)
            $col3W = $W - $cx - 8;
            ...
            $pdf->SetFont('Arial', 'B', 9);
            $pdf->SetXY($cx, $cy);
            $pdf->Cell($col3W, 5, $bagLabel, 0, 0, 'R');
            
            // Weight
            if (!empty($bag['weight_kg'])) {
                $pdf->SetFont('Arial', 'B', 7);
                $pdf->SetXY($cx, $cy + 7);
                $pdf->Cell($col3W, 4, $bag['weight_kg'] . ' kg', 0, 0, 'R');
            }
            
            // Bag count
            $pdf->SetFont('Arial', '', 6);
            $pdf->SetXY($cx, $cy + 12);
            $pdf->Cell($col3W, 3, 'Bag ' . $page . '/' . $bagCount, 0, 0, 'R');
        }
...
public function boardingPass(int $uploadId, array $forceTicketIds = [])
{
    ...
    if ($schedule && !empty($schedule['date'])) {
        ...
        $boardingTime = date('H:i', strtotime($schedule['date'])) . ' WIB';
    }
    ...
    */
    // 210mm x 74mm landscape (boarding pass strip — lebar penuh)
    $pageW = 210;
    $pageH = 74;
    ...
    */
    ...
    foreach ($tickets as $ticket) {
        ...
        // ── Left color strip (vertical) ──────────────────────
        $stripW = 11;
        $pdf->SetFillColor(...$headerColor);
        $pdf->Rect($cardX, $cardY, $stripW, $cardH, 'F');

        // Yacht icon on strip
        $pdf->YachtIconAuto($cardX + 1.5, $cardY + 2, 7.5, 7);

        // Rotated "Boarding Pass" text on strip
        $pdf->SetTextColor(255, 255, 255);
        $pdf->SetFont('Arial', 'B', 7);
        $stripCY = $cardY + ($cardH / 2) + 8;
        $pdf->RotatedText($cardX + $stripW - 2.5, $stripCY, 'Boarding Pass', 90);
        $pdf->SetTextColor(0, 0, 0);

        // ── QR Code (right side) ─────────────────────────────
        $qrSize = $cardH - 6;  // ~62mm — fills height
        $qrX    = $cardX + $cardW - $qrSize - 3;
        $qrY    = $cardY + 3;
        ...
        $divX = $qrX - 4;
        $pdf->DottedLine($divX, $cardY + 2, $divX, $cardY + $cardH - 2, 0.3, 1.5);

        // ── Info area (between strip and QR) ─────────────────
        $infoX  = $cardX + $stripW + 4;
        $infoW  = $divX - $infoX - 3;
        $halfW  = ($infoW - 3) / 2;
        $col2X  = $infoX + $halfW + 3;
        ...
        // Boat name + KET top right of info area
        $pdf->SetFont('Arial', 'B', 7);
        ...
        // GROUP + PASSENGER (side by side)
        $h1 = $fieldStacked($infoX, $y, $halfW, 'Grup', $groupName, [26,26,26], 5.0, 7.5);
        $h2 = $fieldStacked($col2X, $y, $halfW, 'Penumpang', $passengerName, [26,26,26], 5.0, 7.5);
        $y += max(9, max($h1, $h2) + 1.5);

        // DATE | BOARDING (side by side)
        $h1 = $fieldStacked($infoX, $y, $halfW, 'Tanggal', $formattedDate, [26,26,26], 5.0, 7.5);
        $h2 = $fieldStacked($col2X, $y, $halfW, 'Jam', $boardingTime, [26,26,26], 5.0, 7.5);
        $y += max(9, max($h1, $h2) + 1.5);

        // FROM | TO (side by side)
        $h1 = $fieldStacked($infoX, $y, $halfW, 'Dari', $origin, [26,26,26], 5.0, 7.5);
        ...
        if ($captainName) {
            $footerY = $cardY + $cardH - 4.5;
            $pdf->SetDrawColor(...$borderGray);
            $pdf->SetLineWidth(0.2);
            $pdf->Line($infoX, $footerY - 1, $divX - 2, $footerY - 1);
            $pdf->SetFont('Arial', '', 5.5);
            $pdf->SetTextColor(150, 155, 160);
            $pdf->SetXY($infoX, $footerY);
