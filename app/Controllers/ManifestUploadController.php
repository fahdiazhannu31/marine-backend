...
class ManifestUploadController extends ApiController
{
    ...
    public function baggageTagPdf(int $id)
    {
        ...
        $tripDateFmt = $tripDate
            ? strtr(date('d F Y', strtotime($tripDate)), $months)
            : 'N/A';
        $dirLabel = ($direction === 'RETURN') ? 'KEPULANGAN' : 'KEBERANGKATAN';
        ...
        // ── PDF: 100 x 150 mm portrait ───────────────────────────────
        $pdf = new \App\Libraries\BoardingPassPDF('P', 'mm', [100, 150]);
        ...
        for ($page = 1; $page <= $bagCount; $page++) {
            ...
            $pdf->Rect(2, 2, $W - 4, 22, 'F');

            // Yacht icon in header
            $pdf->SetFillColor(...$headerTextColor);
            $pdf->SetDrawColor(...$headerTextColor);
            $pdf->YachtIcon(4, 3, 14, 12, ...$headerTextColor);

            // "NAMA Marine" brand
            $pdf->SetFont('Arial', 'B', 11);
            $pdf->SetTextColor(...$headerTextColor);
            $pdf->SetXY(20, 5);
            $pdf->Cell(55, 6, 'NAMA Marine', 0, 1, 'L');

            // Direction badge pill
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->SetXY(20, 12);
            $pdf->Cell(55, 5, $dirLabel, 0, 0, 'L');

            // Bag counter top-right (e.g. "2 / 3")
            $pdf->SetFont('Arial', 'B', 8);
            $pdf->SetXY($W - 22, 5);
            $pdf->Cell(18, 6, $page . ' / ' . $bagCount, 0, 0, 'R');
            ...
            $pdf->SetXY(5, $y);
            $pdf->MultiCell($W - 10, 7, strtoupper($bag['group_name'] ?? ''), 0, 'L');
            $y = $pdf->GetY() + 1;
            ...
            if ($origin || $destination) {
                ...
                $pdf->SetXY(5, $y);
                $pdf->Cell($W - 10, 5, $routeText, 0, 1, 'L');
                $y = $pdf->GetY() + 1;
            }
            ...
            $fields = [
                ['Kapal',       strtoupper($boatName)],
                ['Tanggal',     $tripDateFmt],
                ['Label / Tag', strtoupper($bag['bag_label'] ?? '-')],
                ['Jumlah Tas',  ($bag['bag_count'] ?? 1) . ' tas'],
            ];
            if (!empty($bag['weight_kg'])) {
                $fields[] = ['Berat', $bag['weight_kg'] . ' kg'];
            }
            if (!empty($bag['description'])) {
                $fields[] = ['Keterangan', $bag['description']];
            }
            ...
            foreach ($fields as [$label, $val]) {
                ...
                $pdf->SetXY(5 + $labelW, $y);
                $pdf->MultiCell($valW, 5, $val, 0, 'L');
                $y = max($y + 5, $pdf->GetY());
            }
            ...
            $barcodeY = $H - 16;
            $pdf->SetFillColor(...$brandColor);
            $pdf->Rect(2, $barcodeY, $W - 4, 14, 'F');

            // Barcode label text
            $bagLabel = strtoupper($bag['bag_label'] ?? ('BAG-' . $bag['upload_id'] . '-' . str_pad($id, 4, '0', STR_PAD_LEFT)));
            $pdf->SetFont('Arial', 'B', 8);
            $pdf->SetTextColor(...$headerTextColor);
            $pdf->SetXY(5, $barcodeY + 1.5);
            $pdf->Cell(60, 5, $bagLabel, 0, 0, 'L');

            // Barcode visual (fake bars using characters)
            $pdf->SetFont('Arial', '', 7);
            $pdf->SetXY(5, $barcodeY + 7);
            $pdf->Cell(60, 4, '||| || ||| | |||| || | ||| || |||| | ||| ||', 0, 0, 'L');

            // Page number right side of barcode
            $pdf->SetFont('Arial', 'B', 7);
            $pdf->SetXY($W - 22, $barcodeY + 2);
            $pdf->Cell(18, 5, 'Tas ' . $page, 0, 0, 'R');
        }
